...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/14 15:15:02 by smishos           #+#    #+#             */
/*   Updated: 2024/10/14 15:15:03 by smishos          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	allocate_cmd_args_arrays(t_pipex *pipex, char **cmd_tokens)
{
	...
	if (pipex->i == 0)
	{
		...
		if (!pipex->cmd_args1)
			error_exit(pipex, cmd_tokens[0], \
				"Failed to allocate memory for cmd_args1\n", 1);
	}
	if (pipex->i == 1)
	{
		while (cmd_tokens[arg_count])
		...
		if (!pipex->cmd_args2)
			error_exit(pipex, cmd_tokens[0], \
				"Failed to allocate memory for cmd_args2\n", 1);
	}
	return (arg_count);
}
...
void	allocate_cmd_args(t_pipex *pipex, char *full_path, char **cmd_tokens)
{
	...
	if (pipex->i == 0)
	{
		pipex->cmd_args1[0] = ft_strdup(full_path);
		pipex->cmd_args1[arg_count] = NULL;
	}
	else
	{
		pipex->cmd_args2[0] = ft_strdup(full_path);
